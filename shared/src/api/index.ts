...
import { OpenRouterPayload, CreditUpdateResult, BirthChartData, PlanetName, ZodiacSign, Position, PlanetPosition, HouseData, AspectData, PatternPlanetData, PatternVisualization, PatternData, SpecialFeature } from "../types";
...
function createTransitSystemPrompt(birthChart?: { planets?: Record<PlanetName, { sign: ZodiacSign; degree: number; house: number }> }): string {
...
async function getAstrologicalTransits(
  ...
  userProfile?: {
    birth_chart?: {
      planets?: Record<PlanetName, { sign: ZodiacSign; degree: number; house: number }>;
    };
  }
): Promise<string> {
...
export async function generateAIResponse(
  ...
  userProfile?: {
    ...
    birth_chart?: {
      planets?: Record<PlanetName, { sign: ZodiacSign; degree: number; house: number }>;
      transits?: {
        current_planets?: Record<PlanetName, { sign: ZodiacSign; degree: number }>;
        aspects?: Array<{
          planet1: string;
          planet2: string;
          aspect: string;
...
${Object.entries(planets)
  ...
  .map(([planetName, data]) => 
    `- ${planetName.charAt(0).toUpperCase() + planetName.slice(1)}: ${data.sign} (${data.degree}°) in House ${data.house}`
  ).join('\n')}
