...
import { updateUserCredits as updateCredits, checkUserCredits as checkCredits } from "../utils";
...
function createTransitSystemPrompt(birthChart) {
...
async function makeOpenRouterRequest(messages, systemPrompt, model, apiKey) {
    const payload = {
        model,
        messages: [
            {
...
async function getAstrologicalTransits(question, apiKey, userProfile) {
    ...
    const response = await makeOpenRouterRequest([{ role: "user", content: `Research current and upcoming planetary transits relevant to this question: ${question}` }], systemPrompt, "google/gemini-2.0-flash-thinking-exp:free", apiKey);
    return response.content;
}
export async function generateAIResponse(messages, userId, userProfile) {
    ...
    try {
        ...
        const updatedSystemPrompt = CHAT_SYSTEM_PROMPT.replace(/Current Date and Time:.*?\n/, `Current Date and Time: ${currentDateTime.date} at ${currentDateTime.time}\n`);
...
${Object.entries(planets)
                .filter(([planetName, data]) => data)
                .map(([planetName, data]) => `- ${planetName.charAt(0).toUpperCase() + planetName.slice(1)}: ${data.sign} (${data.degree}°) in House ${data.house}`).join('\n')}

${userProfile.birth_chart.transits?.aspects ? `
Aspects:
${userProfile.birth_chart.transits.aspects
                .map((aspect) => `- ${aspect.planet1} ${aspect.aspect} ${aspect.planet2} (orb: ${aspect.orb}°)`).join('\n')}
` : ''}
...
` : ''}`;
        ...
        const response = await makeOpenRouterRequest(messages, updatedSystemPrompt + (birthChartContext ? `\n\nCurrent User's Information:\n${birthChartContext}` : '') + transitInfo, "qwen/qwen-plus", apiKey);
        // Update user credits
        const creditUpdate = await updateCredits(userId, response.usage);
