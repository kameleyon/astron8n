...
// Import the appropriate PDF generator based on the environment
// During build time, use the mock implementation to avoid issues with pdf-lib
// At runtime, use the real implementation
const createPDF = process.env.NODE_ENV === 'production' 
  ? require('@/lib/utils/pdf-generator-mock').createPDF 
  : require('@/lib/utils/pdf-generator').createPDF;
...
export async function POST(req: Request) {
  try {
    ...
    const fetchAstrologicalEvents = async (supabase: any, startDate: Date, endDate: Date) => {
      ...
      const calculateHousePosition = (degrees: number, minutes: number, sign: string, birthChart: any) => {
        // Convert to decimal degrees
        const decimalDegrees = degrees + (minutes / 60);
        
        // Calculate absolute position
        const absoluteDegrees = getSignIndex(sign) * 30 + decimalDegrees;
        
        // Find house by comparing with house cusps (which are already in absolute degrees)
      ...
      // Helper function to check for aspects with variable orbs
      const checkAspects = (transitDegrees: number, transitMinutes: number, transitSign: string, birthChart: any) => {
        const aspects: string[] = [];
        
        // Convert transit position to decimal degrees
        const transitDecimalDegrees = transitDegrees + (transitMinutes / 60);
        const transitAbsDegrees = getSignIndex(transitSign) * 30 + transitDecimalDegrees;

        birthChart.planets.forEach((planet: any) => {
          // Use planet.longitude directly as it's already in absolute degrees
      ...
      transitEvents.forEach((event: any) => {
        ...
        // Format event with planet names and aspects (including minutes)
        let formattedEvent = '';
        const transitAspects = checkAspects(event.degrees, event.minutes, event.sign, combinedData.birth_chart);
